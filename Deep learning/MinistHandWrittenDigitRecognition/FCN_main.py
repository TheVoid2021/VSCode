...
# 使用DataLoader加载数据集，设置shuffle为True，表示每次迭代时都会打乱数据集，batch_size为64，表示每次迭代加载64个样本
train_loader = DataLoader(train_data,shuffle=True,batch_size=64)

# 测试数据集
test_data = MNIST(root='./data',train=False,download=False,transform=transforms.ToTensor())
# 使用DataLoader加载数据集，设置shuffle为False，表示每次迭代时不会打乱数据集，batch_size为64，表示每次迭代加载64个样本
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.linear1 = nn.Linear(784,256) # 输入层到隐藏层1的线性变换
        self.linear2 = nn.Linear(256,64) # 隐藏层1到隐藏层2的线性变换
        self.linear3 = nn.Linear(64,10) # 隐藏层2到输出层的线性变换，10个手写数字对应的10个输出

    def forward(self,x):
        x = x.view(-1,784) # 变形，将输入的图片展平为一维向量
        x = torch.relu(self.linear1(x)) # 隐藏层1的激活函数为ReLU
        x = torch.relu(self.linear2(x)) # 隐藏层2的激活函数为ReLU
        x = torch.relu(self.linear3(x)) # 输出层的激活函数为ReLU
        return x
