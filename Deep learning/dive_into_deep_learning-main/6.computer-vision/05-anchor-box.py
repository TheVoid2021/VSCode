...
def multibox_prior(data, sizes, ratios):
    ...
    device, num_sizes, num_ratios = data.device, len(sizes), len(ratios)  # 获取设备、大小和比例的数量
    boxes_per_pixel = (num_sizes + num_ratios - 1)  # 每个像素的锚框数量
    size_tensor = torch.tensor(sizes, device=device)  # 将大小转换为张量
    ratio_tensor = torch.tensor(ratios, device=device)  # 将比例转换为张量
