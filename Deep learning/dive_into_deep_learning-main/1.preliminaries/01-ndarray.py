...
# 使用 arange 创建一个行向量 x，其中包含从 0 到 11 的数字
x = torch.arange(12)
print('x:', x)
...
print(x / y)
print(x ** y)  # （**运算符是求幂运算）
print(torch.exp(x)) # exp函数是指数运算

X = torch.arange(12, dtype=torch.float32).reshape(3, 4)
Y = torch.tensor([[2.0, 1, 4, 3], [1, 2, 3, 4], [4, 3, 2, 1]])
print('cat操作 dim=0', torch.cat((X, Y), dim=0))
print('cat操作 dim=1', torch.cat((X, Y), dim=1))  # 连结(拼接)（concatenate） ,将它们端到端堆叠以形成更大的张量。
...
print('X[1:3]:', X[1:3])  # 用 [1:3] 选择第一行和第二行元素

X[1, 2] = 9  # 写入元素。 行列都是从0开始
print('X:', X)

X[0:2, :] = 12  # 写入元素。第一行和第二行的所有元素写成12
print('X:', X)

print('5.节约内存')
before = id(Y)  # id()函数提供了内存中引用对象的确切地址 类似C++的指针
Y = Y + X
print(id(Y) == before)  # False Y之前的数据丢失了

before = id(X)
X += Y
print(id(X) == before)  # 如果后续计算中没有重复使用X，我们可以使用 X[:] = X + Y 或 X += Y 来减少操作的内存开销。
